/**
 * Adapted from terra-draw (https://github.com/JamesLMilner/terra-draw)
 * MIT License, Copyright (c) James Milner
 */
import {
...
import { StoryArgs, Modes } from './config';

// Minimal structural type standing in for terra-draw's TerraDrawBaseDrawMode,
// which is not exported from the package root
type TerraDrawBaseDrawMode = { mode: string };
...
export function setupMapContainer(args: StoryArgs & { adapter: string }): {
	...
	const modes: Modes[] = args.modes.map((mode) => mode());
...
function createButton({
	...
	button.style.border = background ? `2px solid ${background}` : `2px solid ${COLORS.border}`;
...
const modeToLabel = (mode: TerraDrawBaseDrawMode) => {
	return mode.mode
		.split('-')
...
function getModeId(mode: TerraDrawBaseDrawMode) {
...
export function whenElementExists(selector: string, cb: (el: HTMLElement) => void) {
	function check() {
