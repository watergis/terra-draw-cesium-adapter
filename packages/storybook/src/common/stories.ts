...
import { DefaultSize, LocationNewYork, DefaultZoom, Story, DefaultPlay } from './config';
	...
	args: {
		...
		modes: [
			() =>
				new TerraDrawMarkerMode({
					styles: {
						markerUrl: 'https://upload.wikimedia.org/wikipedia/commons/1/1e/Marker_location.jpg',
						markerWidth: 226 / 10,
						markerHeight: 393 / 10
					}
				})
	...
	args: {
		...
		afterRender: (draw: TerraDraw) => {
			draw.on('change', (ids) => {
				...
				if (feature) {
					const provisionalCount = feature.properties['provisionalCoordinateCount'];
			...
			() =>
				new TerraDrawPolygonMode({
					snapping: {
						toFeature: {
							filter: (feature) => {
								return feature.geometry.type === 'Point' || feature.geometry.type === 'LineString';
							},
							toLine: true,
							toCoordinate: true
						}
				...
				new TerraDrawPolygonMode({
					styles: {
						fillColor: (feature: GeoJSONStoreFeatures) =>
							(feature.properties.randomColor as HexColor) || ('#ff0000' as HexColor)
					}
				})
		],
		...DefaultStory.args,
